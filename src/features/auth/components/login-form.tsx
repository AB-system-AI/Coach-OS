...
import { getAuthProviders, getPostLoginDestination } from "@/features/auth/actions/redirect-actions";
import { isUnverifiedAuthError } from "@/lib/auth/email-verification";
...
function LoginFormInner({
  ...
  useEffect(() => {
    void getAuthProviders().then((providers) => {
      setGoogleAuthEnabled(providers.google);
    });
  }, []);
  ...
  async function handlePasswordSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result?.error) {
        if (isUnverifiedAuthError(result.error)) {
  ...
  async function handleGoogleSignIn() {
    if (!googleAuthEnabled) {
      toast.error("Google sign-in is not configured yet. Use email and password instead.");
      return;
    }
